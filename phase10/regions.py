...
def create_and_connect_regions(world: Phase10World) -> None:
    ...
    store = Region("Store", world.player, world.multiworld)
    victory = Region("Victory", world.player, world.multiworld)
    world.multiworld.regions += [menu, table, store, *phases, victory]
    ...
    # The Store needs nothing either: what gates it is the price on each slot,
    # not the door. An entrance rule here would only duplicate them.
    menu.connect(store, "Menu to Store")
